...
coeficientes_joback = {
    # Diccionario con los coeficientes de Joback
...
st.title('Calculadora de Calor Específico')

st.header('Ingrese los grupos funcionales y sus cantidades')

grupos_funcionales = {}
for grupo in coeficientes_joback.keys():
    cantidad = st.number_input(f'{grupo}', min_value=0.0, step=0.1, format="%.1f")
    if cantidad > 0:
        grupos_funcionales[grupo] = cantidad

temperatura_min = st.number_input('Temperatura mínima', min_value=-273.15, step=1.0, format="%.1f")
temperatura_max = st.number_input('Temperatura máxima', min_value=-273.15, step=1.0, format="%.1f")

if st.button('Calcular'):
    if temperatura_min >= temperatura_max:
        st.error('Error: La temperatura mínima debe ser menor que la temperatura máxima.')
    else:
        resultado, formula = calcular_cp_integral(grupos_funcionales, temperatura_min, temperatura_max)
        if resultado is None:
            st.error(f"Error: {formula}")
        else:
            st.success(f'Entalpía: {resultado}')
